...
class PostController extends Controller
{
    ...
    public function __construct() {
        ...
        $this->middleware(['permission:create_post'])->only('store', 'create');
    ...
    public function index(){
        $posts = Post::with('user:id,name', 'category:id,name')->withOut('profile')->latest();

        $user_id = auth()->user()->id;
        $user = User::find($user_id);
        return view('pages.back-end.post.index', compact('posts'))->with('postAll', $user->posts);
    }

    public function index2(){
        $posts = Post::with('user:id,name', 'category:id,name')->withOut('profile')->latest()->paginate(15);
        return view('pages.back-end.post.index2', compact('posts'));
    }
    ...
    public function edit($id)
    {
        ...
        if(auth()->user()->id === $post->user_id OR $role->id == '1'){
            return view('pages.back-end.post.edit', compact('post'));
        }else{
            return redirect()->Route('post.index2')->with('error', 'Unauthorized Page');
